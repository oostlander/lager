...
bool avl_gleich(struct SchluesselTyp s1, struct SchluesselTyp s2)
{
	...
	{
		if (strcmp(s1.Bezeichner, s2.Bezeichner)){
			if (*s1.Elementnummer == *s2.Elementnummer){
				result = true;
			}
			else{
				result = false;
...
bool kleiner(struct SchluesselTyp s1, struct SchluesselTyp s2)
{
	...
	if (*s1.Teilenummer < *s2.Teilenummer){
		...
		if (strcmp(s1.Bezeichner, s2.Bezeichner) < 0){
			result = true;
		}
		else{
			if (*s1.Elementnummer < *s2.Elementnummer){
...
{
	printf("T %i ", a->avl_daten.Teilenummer);
	printf("B %s ", a->avl_daten.Bezeichner);
	printf("E %i ", a->avl_daten.Elementnummer);
	printf("Br %f ", a->avl_daten.Breite);
	printf("G %f ", a->avl_daten.Gewicht);
	printf("\n"); 
}
...
AVL_knoten *avl_suchen(struct SchluesselTyp s, AVL_baum b)
{
	...
	if (b == NULL) // Suche erfolglos beendet 
		...
		if (avl_gleich(b->avl_daten.schluessel, s))
		{
...
/* ------------------------------------------------ */
// Loescht den "am weitesten links stehenden" also den ersten Knoten mit Schlüssel s
